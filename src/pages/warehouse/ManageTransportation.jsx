...
function ManageTransportation() {
    ...
    const updateFunction = (newRow) => {
        ...
        ({
            ...prev,
            receviedProduct: prev.receviedProduct.map((row) => (row.id === newRow.id ? newRow : row))
        })
        );
    };
    ...
    const getTransportation = async (link = `/products/transportations/${transportationId}`) => {
        try {
            setLoading(true);
            setError(null);
    ...
    const handleRecevie = async () => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                axiosPrivate
                    .post(`/products/transportations/${transportationId}/receive`, { code: code })
                    .then(() => {
                        Swal.fire({
                            title: "تمت عملية الإرسال بنجاح",
                            icon: "success",
    ...
    const handleConfirm = async () => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                const received_products = productsTransportation.receviedProduct.map(product => ({
                    product: product.id,
                    quantity: product.quantity
                }))
                axiosPrivate
                    .post(`/products/transportations/${transportationId}/confirm`, { received_products: received_products })
                    .then(() => {
                        Swal.fire({
                            title: "تمت عملية الإرسال بنجاح",
                            icon: "success",
                ...
                {loading ? (
                    ...
                    <div className="relative w-full flex flex-col">
                        <div className="flex items-start justify-end py-5">
                            <p className="relative text-center">{productsTransportation.source}</p>
                            <p className="relative text-end w-[250px]">:ارسال من فرع</p>
                        </div>
                        <div className="flex items-start justify-end py-5 ">
                            <p className="relative text-center">{productsTransportation.destination}</p>
                            <p className="relative text-end w-[250px] ">:ارسال الى فرع</p>
                        </div>
                        <div className="flex items-start justify-end py-5">
                            <p className="relative text-center">{productsTransportation.date}</p>
                            <p className="relative text-end w-[250px]">:تاريخ الانشاء</p>

                        </div>
                        <div className="flex items-start justify-end py-5">
                            <p className="relative text-center">{productsTransportation.transported_at}</p>
                            <p className="relative text-end w-[250px]">:تاريخ الارسل</p>
                        </div>
                        <div className="flex items-start justify-end py-5">
                            <p className="relative text-center">{productsTransportation.received_at}</p>
                            <p className="relative text-end w-[250px]">:تاريخ التلقي</p>

                        </div>
                        <div className="flex items-start justify-end py-5">
                            <p className="relative text-center" style={{ color: STATUSES[productsTransportation.status] }}>{productsTransportation.status}</p>
                        ...
                        </div>
                        {
                            ...
                            <div className="flex items-start justify-end py-5">
                                <p className="relative text-center" >{productsTransportation.code}</p>
                        ...
                        <div className="flex flex-col items-end justify-end py-5 ">
                            ...
                            <p className="relative text-end w-[250px] py-5">:المنتجات المرسلة </p>
                            <DataTable
                        ...
                        </div>
                        {
                            ...
                            <div className="flex flex-col items-end justify-end py-5 ">
                                ...
                                <p className="relative text-end w-[250px] py-5">:المنتجات المستلمة </p>
                        ...
                        }
                        {
                            ...
                            <div className="flex items-start justify-end py-5">
                                <TextInputComponent value={code} onChange={value => setCode(value)} />
                                <p className="relative text-end w-[250px]">:كود تأكيد التوصيل </p>
                        ...
                        <div className="relative flex justify-between">
                            ...
                            {
                                productsTransportation.status != "confirmed" &&
                                <ButtonComponent variant={NEXT_STATUSE[productsTransportation.status]} onClick={NEXT_ACTION[productsTransportation.status]} />
                            }
                        </div>
