...
function AddWorkSchedule() {
  ...
  const location = useLocation();
    ...
    const addWorkSchedule = async () => {
        Swal.fire({
            title: "هل أنت متأكد من عملية اضافة جدول الأعمال",
            icon: "warning",
            ...
            if (result.isConfirmed) {
                axiosPrivate
                    .post(`/employees/work_schedules`, workSchedule)
                    .then(() => {
                        Swal.fire({
        ...
        <main className="flex flex-col items-center justify-center w-full h-full flex-grow gap-4 ">
            ...
            <section className="flex items-center justify-center flex-col gap-16 w-full bg-white rounded-[30px] py-8 px-4 my-box-shadow">
                ...
                <div className="relative w-full flex flex-col justify-center items-center">
                    ...
                    <DataTable dir={'rtl'} columns={WorkSchedulesDayesColumns} rows={workSchedule.work_days} />
                </div>
                <div className="flex items-center justify-end gap-4 w-full">
                    <ButtonComponent variant={"back"} onClick={() =>navigate(-1) } />
                    <ButtonComponent variant={"add"} onClick={() => {addWorkSchedule()}} />
                </div>
